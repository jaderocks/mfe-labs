...
class APIError extends Error {
    constructor(message){
        super(`${message}` || 'Not a valid entitlement');
    }
}
class APIClient {
    static async eval(script) {
        if (typeof script !== "string") return Promise.reject(new TypeError('not a script'));
        try {
            const request = {
                apiVersion,
                endpoint: 'Evaluate script',
                body: {
                    xml: script
                }
            }, response = await this._sendRequest(request);
            if (response.status.code !== 200) {
                return Promise.reject(new APIError(`API response was: ${response.status.code} ${response.status.message}`));
            }
            let result;
            if (response.body.webRequest) {
                result = QworumRequest.fromJsonable(response.body.webRequest);
            } else if (response.body.data) {
                result = DataValue.fromIndexedDb(response.body.data);
                if (!(result instanceof Json || result instanceof SemanticData)) {
                    return Promise.reject(new TypeError('received unknown data type'));
                }
            } else {
                try {
                    result = Fault.fromIndexedDb(response.body.fault);
                } catch (_error1) {
                    try {
                        result = PlatformFault.fromIndexedDb(response.body.fault);
                    } catch (_error) {}
                }
            }
            if (!result) return Promise.reject(new APIError('unparsable result'));
            return Promise.resolve(result);
        } catch (error) {
            return Promise.reject(new APIError(`APIClient.eval() returned an error: ${error}`));
        }
    }
    static closeTab() {
        try {
    ...
    static async logNavigation(referrerUrl) {
        if (referrerUrl && (typeof referrerUrl !== 'string' || referrerUrl.length === 0)) return Promise.reject(new TypeError('Bad "referrerUrl" argument for APIClient.logNavigation()'));
        if (referrerUrl) {
            try {
                new URL(referrerUrl);
            } catch (error) {
                return Promise.reject(new TypeError(`Bad "referrerUrl" argument for APIClient.logNavigation(): ${error}`));
            }
        }
        try {
            const request = {
                apiVersion,
                endpoint: 'Log page navigation',
                body: {
                    referrerUrl
                }
            }, response = await this._sendRequest(request);
            console.info(`api client logNavigation (response: ${JSON.stringify(response)})`);
            if (response.status.code !== 200) {
                return Promise.reject(new APIError(`API error: ${response.status.code} ${response.status.message}${response.body.message ? `, ${response.body.message}` : ''}`));
            }
            let result = null;
            if (response.body.webRequest) {
                result = QworumRequest.fromJsonable(response.body.webRequest);
                ...
                result = DataValue.fromIndexedDb(response.body.data);
                if (!(result instanceof Json || result instanceof SemanticData)) {
                    return Promise.reject(new APIError('APIClient.logNavigation() received unknown data type'));
                }
            } else if (response.body.fault) {
                try {
                    result = Fault.fromIndexedDb(response.body.fault);
                } catch (_error1) {
                    try {
                        ...
                        return Promise.reject(new APIError('APIClient.logNavigation() received unknown fault type'));
                    }
                }
            }
            return Promise.resolve(result);
        } catch (error1) {
            return Promise.reject(new APIError(`APIClient.logNavigation() returned an error: ${error1}`));
        }
    }
    ...
    static async getData(path) {
        ...
        try {
            ...
            return Promise.resolve(DataValue.fromIndexedDb(response.body.value));
        } catch (error) {
            return Promise.reject(error);
        }
    }
    static async checkQworumAvailability() {
        try {
            const request = {
                apiVersion,
                endpoint: 'Check Qworum availability',
                body: {}
            };
            await this._sendRequest(request);
            return Promise.resolve();
        } catch (_error) {
            return Promise.reject(new APIError(`The browser isn't Qworum-enabled.`));
        }
    }
    static async _sendRequest(request) {
        console.info(`[api client send request] request: ${JSON.stringify(request)}`);
        try {
            const response = await chrome.runtime.sendMessage(request);
            return Promise.resolve(response);
        } catch (error) {
            return Promise.reject(new APIError(`[APIClient] API request returned an error: ${error}`));
        }
    }
...
// export { DataValue as DataValue, GenericData as GenericData, Json as Json, SemanticData as SemanticData, Instruction as Instruction, Fault as Fault, FaultTypeError as FaultTypeError, PlatformFault as PlatformFault, PlatformFaultTypeError as PlatformFaultTypeError, Return as Return, Sequence as Sequence, Data as Data, Try as Try, Goto as Goto, Call as Call, Script as Script, QworumRequest as QworumRequest, APIClient as APIClient };


// end of mod.mjs

// WARNING Content scripts aren't allowed to import modules.
// import {
//     // Script, 
//     DataValue, Json, SemanticData, 
//     Fault, PlatformFault,
//     Script,
//     QworumRequest,
//     APIClient
// } from './modules/mod.mjs';

// modify the extension icon for websites where Qworum is activated
...
addEventListener('load', (_event) => {
    //     console.info('[content script] DOM fully loaded and parsed');
    new ContentScript().run();
});

/**
 * Class that encapsulates all content script operations.
 */
class ContentScript {
    /**
     * The referrer of the current page. Any existing Qworum session will be terminated
     * by the extension's service worker if the current page has no referrer.
     * @type {string | null}
     */
    referrer = null;

    /**
     * If the current page is an XML document, than this property will contain the document.
     * If the XML isn't a Qworum script, then the extension's service worker will raise a script fault.
     * @type {string | undefined}
     */
    xml;

    /**
     * The content script must execute this to create an instance when the web page is loaded.
     */
    constructor(){
        // Detect the referrer
        this.referrer = (
        ...
        if (
            ...
            const xmlViewerDiv = document.getElementById('webkit-xml-viewer-source-xml');
    
            if (xmlViewerDiv instanceof Element && xmlViewerDiv.children.length === 1) {
                const xmlRootElement = xmlViewerDiv.children[0];
    
                if (xmlRootElement instanceof Element) {
                    this.xml = new XMLSerializer().serializeToString(xmlRootElement);
                    console.info(`[Content script] Found XML in page: ${this.xml}`);
                }
            }
        }
    }
    ...
    /**
     * The content script must execute this when the web page is loaded.
     */
    async run(){
        let action;
        try {
            // BUG: REFERRERLESS WEB RESPONSES MUST END CURRENT SESSION IF ANY !!!
            // 1. send navigation info
            // action = await APIClient.logNavigation(this.referrer); // action is: QworumRequest | Fault | DataValue | null
            // if(this._handleResponse(action)) return;

            // 2. eval script (only if sending navigation info didn't yield any result)
            if (this.xml) {
                let xml = this.xml;
                try {
                    Script.fromXml(this.xml);
                    // valid Qworum script that does not contain a platform fault
                } catch (error) {
                    // not a valid Qworum script, or it contains a platform fault
                    const script = Script.build(PlatformFault.build('script'));
                    xml = script.toXml();
                    // xml = `<sequence xmlns='https://qworum.net/ns/v1/instruction/' xmlns:data='https://qworum.net/ns/v1/data/'><fault type='script' /></sequence>`;
                }
                action = await APIClient.eval(xml); // action is: QworumRequest | Fault | DataValue
    ...
    /**
     * Performs an action based on the service worker API's response.
     * 
     * @private
     * @param {QworumRequest | Fault | DataValue} action
     * @returns {boolean} True if an action was performed and the content script can terminate its execution.
     */
    _handleResponse(action){
        if(!action) return false;

        // Qworum session continues
        if (action instanceof QworumRequest) {
            window.location.replace(`${action.url}`);
            return true;
        }

        // Qworum session has ended
        let alertMessage = '';
        if (action instanceof Fault) {
            alertMessage = chrome.i18n.getMessage(
                'application_raised_fault', [`${action}`]
            );
        } else { // action instanceof DataValue
            alertMessage = chrome.i18n.getMessage(
                'application_returned_result', [`${action}`]
            );
        }
        alert(alertMessage);
        APIClient.closeTab(); // BUG? can the extension close a tab it hasn't opened itself?
        return true;
    }
...
//                 console.log(`[Qworum content script] ${message}`);
//     },
//     foundXML: false,
//     contentType: document.contentType.toLowerCase(),
//     response: null,
//     handleServiceWorkerResponse: function (response) {
//         // handleServiceWorkerResponseAfterForwarding is obviously executing,
//         // so why are console.logs not showing?
//         try {
//             my.log(`[my.handleServiceWorkerResponse]`);
//             if (response.inSession) {
//                 my.log(`[my.handleServiceWorkerResponse] inSession, forwarding in tab's history...`);
//                 my.response = response;
//                 window.history.forward(); // BUG this does not run when when going back one page in a session (for now, using in-page js library instead)
//                 setTimeout(
//                     () => my.handleServiceWorkerResponseAfterForwarding(my.response),
//                     50 // milliseconds
//                 );
//                 return;
//             }
//             my.handleServiceWorkerResponseAfterForwarding(response);
//         } catch (error) {
//                         console.log(`[my.handleServiceWorkerResponse] error: ${error}`);
//         }
//     },
//     handleServiceWorkerResponseAfterForwarding: function (response) {
//         switch (response.type) {
//             case 'request':
//                 if (response.phaseParameters) {
//                     my.log('service worker response is a POST request; using a form');
...
//                     phaseParametersInput.setAttribute('id', 'qworum-arguments'); // not needed?
//                     phaseParametersInput.setAttribute('name', 'qworum-arguments');
//                     phaseParametersInput.setAttribute('value', response.phaseParameters);
...
//                 } else {
//                     my.log('service worker response is a GET request; redirecting');
...
//             case 'data':
//                 my.log('service worker response is data');
//                 alert('The application has quit normally.');
//                 chrome.runtime.sendMessage(
//                     { type: '[Content script API v1] close tab' },
//                     function (response) {
//                         my.log(`[close tab] message from service worker: ${JSON.stringify(response)}`);
//                     }
//                 );
//                 // window.close();
//                 break;
...
//             case 'fault':
//                 my.log('service worker response is a fault');
//                 alert(`The application has quit with a "${response.faultType}" fault.`);
//                 chrome.runtime.sendMessage(
//                     { type: '[Content script API v1] close tab' },
//                     function (response) {
//                         my.log(`[close tab] message from service worker: ${JSON.stringify(response)}`);
...
// // my.h1.appendChild(document.createTextNode(`[${my.prefix}] Starting`));
// // document.body.appendChild(my.h1);

// my.log(`Content script has started.`);

// window.addEventListener('load', (event) => {
//         console.log('[content script] DOM fully loaded and parsed');
//     runTheContentScript();
// });

// function runTheContentScript() {
//     const referrer = (
//         typeof document.referrer === 'string' && document.referrer.length > 0
//     ) ? document.referrer : null;
...
//     // Detect XML in page
//     if (
//         my.contentType.indexOf('application/xml') !== -1 ||
//         my.contentType.indexOf('text/xml') !== -1
...
//             if (xmlRootElement instanceof Element) {
//                 const xmlString = new XMLSerializer().serializeToString(xmlRootElement);
//                 my.log(`Found XML in page: ${xmlString}`);
//                 my.foundXML = true;

//                 // XML might or might not be a Qworum script
//                 chrome.runtime.sendMessage(
//                     {
//                         type: '[Content script API v1] xml',
//                         url: document.documentURI,
//                         referrer: referrer,
//                         xml: xmlString
//                     },
//                     function (response) {
//                         my.log(`[XML] message from service worker: ${JSON.stringify(response)}`);
//                         my.handleServiceWorkerResponse(response);
//                     }
//                 );
//             }
//         }
...
//     // Page is Not XML
//     if (!my.foundXML) {
//         my.log(`Page is Not XML`);
//         chrome.runtime.sendMessage(
//             {
//                 type: '[Content script API v1] not xml',
//                 url: document.documentURI,
//                 referrer: referrer,
//                 contentType: my.contentType
//             },
//             function (response) {
//                 my.log(`[Not XML] message from service worker: ${JSON.stringify(response)}`);
//                 my.handleServiceWorkerResponse(response);
//             }
//         );
